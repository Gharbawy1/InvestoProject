...
namespace Investo.DataAccess.Services.Project
{
    public class ProjectService : IProjectService
    {
        ...
        public async Task<ValidationResult<ProjectReadDto>> CreateProject(ProjectCreateUpdateDto dto)
        {
            ...
            var User = await _userManager.FindByIdAsync(dto.OwnerId);
            if (User == null) {
                return new ValidationResult<ProjectReadDto>
                {
                    Data = new ProjectReadDto(),
                    ErrorMessage = $"Error While Finding User With Id {dto.OwnerId}",
                    IsValid = false
                };
            }
            var ownerExistsInExpectedRole = await _userManager.IsInRoleAsync(User, "BusinessOwner");
            if (!ownerExistsInExpectedRole) return new ValidationResult<ProjectReadDto>
            {
                Data = new ProjectReadDto(),
                ErrorMessage = "You Have No Permission, Please Upgrade Your Account To BusinessOwner",
                IsValid = false
            };
