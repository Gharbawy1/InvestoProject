...
export class InvestorUpgradeComponent implements OnInit {
  ...
  getFileError(field: string): string {
    ...
    if (control.hasError('invalidType')) return 'Invalid file type. Allowed: JPEG, PNG, PDF';
    if (control.hasError('invalidSize')) return `File size exceeds ${this.maxFileSizeMB}MB`;
